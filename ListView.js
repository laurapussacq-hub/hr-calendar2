...
export default function ListView({ events, onEventClick }) {
  ...
  function renderEvent(event) {
    ...
    const fecha = event.fecha ? format(parseISO(event.fecha), "EEE d MMM yyyy", { locale: es }) : '';
    return (
      <div key={event.id} className="list-event-card" onClick={() => onEventClick && onEventClick(event)}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontWeight: 600, fontSize: 15, color: 'var(--ink)', marginBottom: event.descripcion ? 4 : 0 }}>
              {event.titulo}
            </div>
            {event.descripcion && (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      {upcoming.length > 0 && (
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--ink-muted)', marginBottom: 12 }}>
            Próximos eventos
          </div>
      ...
      {past.length > 0 && (
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--ink-faint)', marginBottom: 12 }}>
            Eventos pasados
          </div>
